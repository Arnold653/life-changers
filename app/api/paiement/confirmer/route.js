...
// Appelée côté client juste après onComplete du widget FedaPay, pour débloquer immédiatement
// sans attendre le webhook (qui reste le filet de sécurité en cas de fermeture du navigateur
// avant que ce callback ne parte). On ne fait JAMAIS confiance à ce que le client affirme : on
// revérifie la transaction directement auprès de FedaPay avant de marquer "reussi".
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Vérification FedaPay indisponible' }, { status: 502 })
  }
