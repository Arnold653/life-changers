...
export default function AdminLivresPage() {
  ...
  const [slugDeverouille, setSlugDeverouille] = useState(false)
    ...
    try {
      ...
      if (type === 'pdf') {
        ...
        setForm((f) => ({
          ...
          sous_titre: f.sous_titre || entete.sousTitre,
  ...
  return (
    <div className="px-6 pt-16 pb-24 max-w-xl mx-auto lever">
      ...
      <p className="text-or text-xs font-mono uppercase tracking-[0.2em] mb-3">Life Changers — Admin</p>
      ...
      <p className="text-papier/45 text-sm mb-10 leading-relaxed">
        {edition
          ...
          : 'Un ouvrage complet livré en un seul fichier (PDF, Markdown ou texte).'}
        <br /><a href="/admin" className="text-or hover:brightness-125">← Retour au tableau de bord</a>
      </p>

      <div className="space-y-6">
        {champ('Titre du livre', 'titre')}
        {champ('Sous-titre (optionnel)', 'sous_titre')}
        {champ('Slug (identifiant dans l\'URL)', 'slug', 'text', !!edition && !slugDeverouille)}
        {edition && !slugDeverouille && (
          <button
            type="button"
            onClick={() => setSlugDeverouille(true)}
            className="text-xs font-mono uppercase tracking-wide text-papier/40 hover:text-or transition-colors -mt-4"
          >
            Modifier quand même (⚠️ casse les liens déjà partagés vers ce livre)
          </button>
        )}
                ...
                {sections.map((s, i) => (
                  <span key={i} className="flex items-center gap-1 font-mono text-[0.65rem] text-papier/50 border border-ligne rounded-full pl-2 pr-1 py-1">
                    ...
                    <PartageSocial
                      ...
                      texteBrut={s.blocs?.slice(1).map((b) => b.texte).join('\n\n')}
                    />
                  </span>
                ))}
              </div>
