...
import { InlineMarkdown } from '@/components/CorpsChapitre'
...
export default function ModifierLivrePage({ params }) {
  ...
  async function sauvegarder() {
    ...
    if (res.ok) {
      ...
      setBlocs(contenuFinal.sections[sectionIndex]?.blocs || [])
    } else {
            ...
            <div key={i} className="border border-ligne rounded-lg p-3 flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                <textarea
                  ...
                  rows={b.titre ? 1 : Math.min(20, Math.max(6, Math.ceil(b.texte.length / 45)))}
                ...
                />
                {!b.titre && b.texte.trim() && (
                  <div className="text-papier/70 text-[0.95rem] leading-relaxed border border-ligne/50 rounded-md px-3 py-2.5 mb-3 bg-encre/40">
                    <p className="text-papier/30 text-[0.6rem] font-mono uppercase tracking-wide mb-1">Aperçu</p>
                    <InlineMarkdown texte={b.texte} />
                  </div>
                )}
