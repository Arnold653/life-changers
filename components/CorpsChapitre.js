...
export function InlineMarkdown({ texte }) {
  ...
  return (
    <>
      {jetons.map((jeton, i) => {
        ...
        if (/^«[^»]+»$/.test(jeton)) return <em key={i} className="text-papier/85">{jeton}</em>
    ...
    <div className={tailleGrande
      ...
      {paragraphes.map((p, i) => {
        ...
        if (p.startsWith('§LISTE§')) {
          let items = []
          try { items = JSON.parse(p.slice('§LISTE§'.length)) } catch { items = [] }
          return (
            <ul key={i} id={`p-${i}`} className="my-2 space-y-1.5 list-disc pl-5 marker:text-or/50">
              {items.map((item, n) => (
                <li key={n} className="text-papier/80 text-[1.02rem] leading-[1.75]">
                  <InlineMarkdown texte={item} />
                </li>
              ))}
            </ul>
          )
