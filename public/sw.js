...
const VERSION = 'life-changers-v1'
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches
      .keys()
      .then((noms) => Promise.all(noms.filter((n) => n.startsWith('life-changers-') && !n.startsWith(VERSION)).map((n) => caches.delete(n))))
