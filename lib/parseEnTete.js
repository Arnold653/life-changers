...
//   # Titre de l'ouvrage
//   Sous-titre: Un sous-titre optionnel     (Livres uniquement)
...
export function extraireEnTeteMetadonnees(texteBrut) {
  ...
  for (; i < lignes.length; i++) {
    ...
    if (!l) continue
    if (/^sous.titre\s*:/i.test(l)) { sousTitre = l.replace(/^sous.titre\s*:/i, '').trim(); continue }
    ...
    // Résumé non étiqueté : format d'origine des prompts ENGINE (Roman/Non-Fiction/Conte...),
    // qui livrent le résumé comme un simple paragraphe juste après le titre, sans le préfixer
    // de "Résumé:". On ne le reconnaît que s'il n'y a pas déjà un résumé capturé, et seulement
    // s'il ne ressemble pas lui-même à un titre — sinon on entre dans le texte réel (break).
    if (lignesResume.length === 0 && !/^#{1,6}\s/.test(l) && !/^(partie|section|page\s+\d|chapitre|d[ée]dicace|avant[- ]propos|introduction|pr[ée]face|prologue|[ée]pilogue|conclusion|remerciements|annexe|bonus)\b/i.test(l)) {
      lignesResume.push(l)
      continue
    }
    break
  ...
  return { titre, sousTitre, genre, region, trancheAge, description: lignesResume.join(' '), resteDuTexte }
}
