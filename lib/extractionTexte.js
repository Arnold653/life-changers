...
export function extraireTexteBrut(brut) {
  ...
  const paragraphes = blocs
    .map((bloc) => {
      ...
      // Tableau Markdown : toutes les lignes du bloc commencent par "|" — reconstruit en
      // tableau de lignes/cellules, la ligne de séparation ("| --- | --- |") est ignorée.
      // §TABLEAU§ est le même marqueur que celui déjà rendu comme un vrai <table> par
      // CorpsChapitre.js.
      if (lignes.length > 1 && lignes.every((l) => l.startsWith('|'))) {
        const lignesTableau = lignes
          .filter((l) => !/^\|?\s*:?-{2,}:?\s*(\|\s*:?-{2,}:?\s*)*\|?$/.test(l))
          .map((l) => l.replace(/^\|/, '').replace(/\|$/, '').split('|').map((c) => c.trim()))
        if (lignesTableau.length > 0) {
          return { type: 'texte', texte: `§TABLEAU§${JSON.stringify(lignesTableau)}`, titre: false, niveau: null }
        }
      }

      // Liste à puces Markdown : toutes les lignes du bloc commencent par "-" ou "*" — sinon
      // elles seraient fondues en un seul paragraphe continu par lignes.join(' ') plus bas,
      // perdant complètement la structure de liste.
      if (lignes.length > 1 && lignes.every((l) => /^[-*]\s+/.test(l))) {
        const items = lignes.map((l) => l.replace(/^[-*]\s+/, '').trim())
        return { type: 'texte', texte: `§LISTE§${JSON.stringify(items)}`, titre: false, niveau: null }
      }
