...
function regrouperEnParagraphes(lignes, tailleCorps) {
  ...
  for (let i = 1; i < lignes.length; i++) {
    ...
    // Seuil calibré sur des exports réels (ex. Google Docs) : l'écart entre deux paragraphes y
    // est souvent d'environ 1.3-1.4x l'interligne normal — nettement moins que le 1.6x utilisé
    // ici avant, qui ratait silencieusement des paragraphes entiers (tout finissait fusionné en
    // un seul bloc dense). 1.3 laisse une marge confortable au-dessus du bruit normal entre
    // lignes d'un même paragraphe (généralement quasi-constant, variations de l'ordre de 1-2%).
    if (imagePresente || ecart > ecartTypique * 1.3) {
      blocs.push(courant)
      courant = [lignes[i]]
    } else {
...
const COUVERTURE_RATIO_MIN = 0.55
...
export async function extrairePdfDepuisUrl(url, televerserImage, onProgression) {
  ...
  for (let i = 1; i <= doc.numPages; i++) {
    ...
    if (televerserImage) {
      let images = await extraireImagesPage(page, pdfjsLib)
